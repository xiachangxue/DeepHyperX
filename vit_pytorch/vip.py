...
class ConvPermuteMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        #w = x.reshape(B, H, W, C).permute(0, 3, 1, 2).reshape(B, C, H, W)
        w = self.mlp_h(x1)

       # c = x.reshape(B, H, W, C).permute(0, 3, 1, 2).reshape(B, C, H, W)
        c = self.mlp_w(x1)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=15, patch_size=3, in_chans=3, embed_dim=256):
        super().__init__()
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=2, padding=1)
